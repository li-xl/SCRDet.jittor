...
def calculate_ious_r(boxes1, box):
    ...
    for i, box1 in enumerate(boxes1):
        ...
        if int_pts is not None:
            ...
            inter = int_area * 1.0 / (area1[i] + area2 - int_area + 1e-5)
        else:
            inter = 0.0
...
def calculate_prec_recall(result,label,iou_thresh,iou_func):
    GTBB = {}
    BB = []
    BB_ids = []
    BB_scores = []
    ...
    for d in range(n_pred):
        ...
        if gt_boxes.shape[0]>0:
            ioumax,jmax = iou_func(gt_boxes,bb)
...
def calculate_VOC_mAP(result,classnames,iou_thresh=0.5,use_07_metric=True):
    ...
    for i,classname in enumerate(classnames):
        ...
        label = i+1
        prec,rec = calculate_prec_recall(result,label,iou_thresh,iou_func=calculate_ious)
        if prec is None:
            all_aps.append(None)
            continue
...
def calculate_VOC_mAP_r(result,classnames,iou_thresh=0.5,use_07_metric=True):
    '''
    INPUTS:
        result: the result list of detections, 
        it's like [(img_id,pred_boxes,pred_labels,pred_scores,gt_boxes,gt_labels),...,(...)]
    ...
    for i,classname in enumerate(classnames):
        # background is not in classnames and it's 0
        label = i+1
        prec,rec = calculate_prec_recall(result,label,iou_thresh,iou_func=calculate_ious_r)
        if prec is None:
            all_aps.append(None)
            continue
        ap = calculate_voc_ap(prec,rec,use_07_metric)
